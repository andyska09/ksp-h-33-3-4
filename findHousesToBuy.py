...
def readjustCoverege(houseCoor, prefixes, k, maxCoor):
    ...
    for row in range(startX, endX):
        if row in houseCoor:
            if startY in houseCoor[row]:
                prefixes[row][startY] = max(0, prefixes[row][startY] - 1)
        difference = prefixes[row][endY] - prefixes[row][startY]
        for j in range(startY, numberCollumns):
            if j < endY:
                prefixes[row][j] = prefixes[row][startY]
            else:
